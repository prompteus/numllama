...
if __name__ == '__main__':
    ...
    p.add_argument(
        'layer',
        choices=range(17, model.config.num_hidden_layers + 1),
        type=int,
    )
    p.add_argument('op', choices=['div', 'mul'])
    args = p.parse_args()
    ...
    best_ckpt = torch.load(f'best_ckpt.gt_probe.{op}.layer{layer_idx}.pt')
    ...
    for idx in range(1, 200_000 + 1):
        ...
        optimizer.step()
        scheduler.step()
    torch.save(patches.detach(), f'patches.gt_probe.{op}.layer{layer_idx}.pt')
