...
df = pd.read_csv('patching_results.mul.csv', index_col=0).reset_index()
...
print(df_.groupby(['alpha', 'layer']).after_ok.mean().reset_index().pivot(index='layer', columns='alpha', values='after_ok') * 100)
