...
for alpha in tqdm.auto.tqdm([0.2, 0.225, 0.25, 0.275, 0.3, 0.325, 0.35, 0.375, 0.4, 0.425, 0.45, 0.475, 0.5]):
    for layer_idx in range(bottom_layer, 29):
        patches = torch.load(f'patches.gt_probe.{op}.layer{layer_idx}.pt').half().to(device)
        for valid_input, valid_label in tqdm.auto.tqdm(list(zip(valid_inputs, valid_labels)), desc=f'L{layer_idx}', leave=False):
...
import pandas as pd
pd.DataFrame.from_records(records).to_csv(f'patching_results.{op}.csv')
