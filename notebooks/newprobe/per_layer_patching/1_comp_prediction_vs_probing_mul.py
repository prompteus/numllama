...
train_labels_ref = torch.tensor([x1 * x2 for _ in range(num_templates) for x1, x2 in train_inputs] + [x1 + x2 for x1, x2 in train_inputs_add], device=device)
...
train_labels_ref = torch.tensor(
    ([x1 * x2 for x1, x2 in train_inputs] * num_templates)
    + [x1 + x2 for x1, x2 in train_inputs_add]
)
...
for layer_idx in reversed(range(len(train_hidden_states))):
    if layer_idx < bottom_layer: break
    torch.manual_seed(0)
    if debug: device ='cpu'
    probe = SinProbeNew(
    ...
    optimizer = torch.optim.Adam(
        [
            ...
            #{"params": probe.parameters(), "weight_decay": 1e-3},
        ],
        lr=1e-4,
    )
    scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=1.0, end_factor=0.1, total_iters=20000)
    ...
    torch.save(best_ckpt, f'best_ckpt.gt_probe.mul.layer{layer_idx}.pt')
...
df.to_csv("gt_vs_neprobes_preds_llama3b_div_040126.csv", index=False)  # TODO: visualize
