...
def main(
    ...
    if limit_num_examples_per_ds is not None and limit_num_examples_per_ds > 0:
        ...
        df = (
            df.groupby("source_ds")
            .apply(lambda g: g.sample(limit_num_examples_per_ds, random_state=seed) if len(g) > limit_num_examples_per_ds else g)
    ...
    with open(output_jsonl, "a") as output_file:
        for batch in examples:
            ...
            inputs_str = [template.format(example[question_column].strip()) for example, template in zip(batch, batch_templates)]
            ...
            for example, template, prediction in zip(batch, batch_templates, preds_str):
                pred_result = numllama.markup.get_result_from_output(prediction)
                ...
                if result_column is not None:
                    ...
                    is_correct = numllama.metrics.are_results_same(true_result, pred_result)
