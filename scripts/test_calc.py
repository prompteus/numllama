...
def main(
    ...
    pred_results = pred_outputs.apply(numllama.markup.get_result_from_output)
    is_correct = numllama.metrics.are_results_same(pred_results, true_results)
...
def print_info(
    ...
    print(
        f"  predictions have a correct final result in {mean:.1%} ± {radius * 100:.1f} of cases. latex: {mean * 100:.1f}±\\small{{{((high - low) / 2) * 100:.1f}}}"
    )
