...
import numllama.datatypes
import numllama.gadget
import numllama.markup
...
def scalar_are_results_same(pred_result: str, true_result: str, rel_tol: float) -> bool:
    ...
    try:
        pred_float = numllama.gadget.Calculator._float_eval(pred_result)
        true_float = numllama.gadget.Calculator._float_eval(true_result)
...
def get_num_gadgets_calls(chain: numllama.datatypes.Chain) -> int:
    return sum(isinstance(step, numllama.datatypes.Interaction) for step in chain)
    ...
    def __call__(self, eval_preds: transformers.EvalPrediction) -> Dict[str, float]:
        ...
        for expected_inputs, actual_inputs in zip(self.expected_input_tokens, input_tokens):
            if expected_inputs != actual_inputs:
                warnings.warn(
                    f"Expected inputs: '{expected_inputs}', but got '{actual_inputs}', it is likely that compute_metrics recieved incorrect `eval_ds_inputs` and `source_ds_col`"
                )
        ...
        df[["trues_chain", "trues_result"]] = df["trues"].apply(numllama.markup.from_model_markup).apply(pd.Series)
        df[["preds_chain", "preds_result"]] = df["preds"].apply(numllama.markup.from_model_markup).apply(pd.Series)
        ...
        logged_dict["avg_correct_results"] = np.mean(
            [logged_dict[f"{source_ds}__correct_results"] for source_ds in df["source_ds"].unique()]
        )
